...
@if ($products->count() > 0)
    ...
    <a href="{{ route('admin.products.edit', $product->id) }}" class="btn-edit-katalog" title="Edit Produk">
        <img src="{{ asset('img/icon/edit-icon.png') }}" alt="Edit" class="action-icon">
    </a>

    <form method="POST" action="{{ route('admin.products.destroy', $product->id) }}"
        onsubmit="return confirm('Apakah Anda yakin ingin menghapus produk ini?')" style="margin: 0;">
        @csrf
        @method('DELETE')
        <button type="submit" class="btn-delete-katalog" title="Hapus Produk">
            <img src="{{ asset('img/icon/hapus-icon.png') }}" alt="Hapus" class="action-icon">
        </button>
    </form>
</div>
