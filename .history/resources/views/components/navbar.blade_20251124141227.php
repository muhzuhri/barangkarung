...
<div id="bk-chatbot" data-token="{{ csrf_token() }}" style="position: fixed; right: 20px; bottom: 20px; z-index: 9999; font-family: 'Poppins', sans-serif;">
    <div id="bk-chatbot-window" style="width: 400px; height: 550px; background: #fff; border: 1px solid #e5e7eb; border-radius: 12px; box-shadow: 0 10px 25px rgba(0,0,0,0.08); display: none; flex-direction: column; overflow: hidden;">
        <div style="background:#111827; color:#fff; padding:14px 16px; display:flex; align-items:center; justify-content:space-between;">
            <div style="font-weight:600; font-size:16px;">Asisten BarangKarung</div>
            <button id="bk-chatbot-close" style="background:transparent; border:none; color:#fff; font-size:20px; cursor:pointer;">×</button>
        </div>
        <div id="bk-chatbot-messages" style="flex:1; padding:16px; overflow-y:auto; background:#f9fafb;"></div>
        <div id="bk-chatbot-quick-replies" style="padding:10px 14px; border-top:1px solid #e5e7eb; background:#fff; display:none; flex-wrap:wrap; gap:6px; max-height:120px; overflow-y:auto;"></div>
        <div style="padding:12px; border-top:1px solid #e5e7eb; background:#fff; display:flex; gap:8px;">
            <input id="bk-chatbot-input" type="text" placeholder="Tulis pertanyaan..." style="flex:1; padding:12px 14px; border:1px solid #e5e7eb; border-radius:8px; font-size:14px;">
            <button id="bk-chatbot-send" style="background:#111827; color:#fff; border:none; border-radius:8px; padding:12px 16px; font-size:14px; cursor:pointer;">Kirim</button>
        </div>
    </div>
    ...
    <script>
        (function(){
            ...
            function formatText(text) {
                ...
                text = text.replace(/(📌\s*)([A-Z\s]+)(<br>)/g, '<div style="margin-top:12px; margin-bottom:8px;"><span style="font-size:18px; vertical-align:middle;">$1</span><strong style="color:#111827; font-size:15px; font-weight:700;">$2</strong></div>');
                
                // Format nomor urut dengan indentasi (   1. pertanyaan)
                text = text.replace(/(<br>)\s{3,}(\d+\.\s)(.+?)(?=<br>|$)/g, '<div style="margin-left:16px; margin-top:4px; margin-bottom:4px; padding-left:8px; border-left:2px solid #e5e7eb;"><strong style="color:#111827;">$2</strong>$3</div>');
                ...
                text = text.replace(/(<br>)\s{3,}(.+?)(?=<br>|$)/g, '$1<span style="display:block; margin-left:16px; padding-left:8px; border-left:2px solid #e5e7eb; margin-top:4px; margin-bottom:4px;">$2</span>');
                
                // Format pertanyaan terkait dengan styling khusus
                text = text.replace(/(💡\s*Pertanyaan terkait:)/g, '<strong style="display:block; margin-top:12px; margin-bottom:8px; color:#111827; font-size:15px;">$1</strong>');
                
                // Format bullet points (▫️) dengan styling
                text = text.replace(/(▫️\s*)(.+?)(?=<br>|$)/g, '<span style="display:block; margin-top:6px; margin-bottom:6px;"><span style="font-size:16px; margin-right:6px;">▫️</span><strong style="color:#111827;">$2</strong></span>');
                
                // Format daftar pertanyaan (📋)
                text = text.replace(/(📋\s*Berikut adalah daftar pertanyaan)/g, '<strong style="display:block; margin-bottom:12px; color:#111827; font-size:15px;">$1</strong>');
            ...
            function greet(){
                appendBubble('Halo! Saya asisten BarangKarung. Tanyakan soal pakaian, ukuran, pengiriman, atau cara belanja ya.\n\n💡 Ketik "bantuan" atau "pertanyaan" untuk melihat daftar pertanyaan yang bisa ditanyakan.','bot');
                loadQuickReplies();
            }
                    ...
                    .then(data => {
                        if (data.faqs && data.faqs.length > 0) {
                            ...
                            data.faqs.forEach(faq => {
                                ...
                                btn.style.cssText = 'padding:8px 12px; font-size:12px; background:#f3f4f6; border:1px solid #e5e7eb; border-radius:6px; cursor:pointer; color:#111827; white-space:nowrap;';
